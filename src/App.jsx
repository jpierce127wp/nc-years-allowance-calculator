...
export default function App() {
  ...
  const [form, setForm] = useState({
    decedentName: '', county: '', fileNumber: '', dateOfDeath: '', domicileInNC: true,
    spouseName: '', spouseAddress: '', spouseCity: '', spouseState: 'NC', spouseZip: '', spousePhone: '',
    hasPR: false, prName: '', prAddress: '', lettersIssuedDate: '',
    assets: [], assigned: {},
  });
  ...
  const eligible = useMemo(() => form.assets.filter(a => {
    const cat = ASSET_CATEGORIES.find(c => c.value === a.category);
    return cat?.eligible !== false && a.isPersonalProperty !== false;
  }), [form.assets]);

  const totalEligible = useMemo(() => eligible.reduce((s, a) => s + (parseFloat(a.value) || 0), 0), [eligible]);
  const totalAssigned = useMemo(() => Object.entries(form.assigned).reduce((s, [id, amt]) => {
    const asset = form.assets.find(a => a.id === id);
    return asset && eligible.includes(asset) ? s + (parseFloat(amt) || 0) : s;
  }, 0), [form.assigned, form.assets, eligible]);
  ...
  const addAsset = () => set('assets', [...form.assets, { id: uid(), description: '', category: 'bank_account', location: '', value: '', isPersonalProperty: true }]);
  
  const updateAsset = (id, field, val) => {
    set('assets', form.assets.map(a => {
      if (a.id !== id) return a;
  ...
  const assign = (id, amt) => {
    ...
    set('assigned', { ...form.assigned, [id]: Math.min(Math.max(0, parseFloat(amt) || 0), max) });
  };
  ...
  const autoAssign = () => {
    ...
    for (const asset of sorted) {
      ...
      if (take > 0) { result[asset.id] = take; rem -= take; }
    }
    set('assigned', result);
  };
  ...
  const validate = (s) => {
    switch (s) {
      case 1: return form.decedentName.trim() && form.county;
      case 2: return form.dateOfDeath;
      case 3: return form.spouseName.trim();
      case 4: return !form.hasPR || (form.prName.trim() && form.lettersIssuedDate);
      case 5: return form.assets.length > 0;
      case 6: return totalAssigned > 0;
      default: return true;
    }
  };
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Source Sans 3', system-ui, sans-serif" }}>
      ...
      <header className="bg-gradient-to-r from-slate-800 to-slate-700 text-white px-6 py-5 shadow-lg">
        <div className="max-w-4xl mx-auto flex justify-between items-center flex-wrap gap-4">
          ...
          <button onClick={clearAll} className="text-sm px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 transition border border-white/20">
            Clear All Data
          </button>
        </div>
      </header>
          ...
          {STEPS.map((s, i) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap ${
                ...
                i < step ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' :
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-6">
          ...
          {step === 6 && <AssignStep form={form} eligible={eligible} totalEligible={totalEligible} totalAssigned={totalAssigned} deficiency={deficiency} assign={assign} autoAssign={autoAssign} clearAssign={clearAssign} />}
          {step === 7 && <ReviewStep form={form} isPost={isPost} formVer={formVer} eligible={eligible} totalAssigned={totalAssigned} deficiency={deficiency} hasDeficiency={hasDeficiency} deadline={deadline} />}
        </div>

        {/* Nav Buttons */}
        <div className="flex justify-between gap-4">
          {step > 0 && (
            <button onClick={prev} className="px-6 py-3 rounded-xl border-2 border-slate-300 text-slate-600 font-semibold hover:bg-slate-50 transition">
              ← Back
            </button>
          )}
          {step < STEPS.length - 1 && (
      ...
      <footer className="bg-amber-50 border-t border-amber-200 px-6 py-4">
        <div className="max-w-4xl mx-auto text-sm text-amber-800">
          <strong>⚠️ Disclaimer:</strong> This calculator is for informational purposes only and does not constitute legal advice. 
          Verify all values and eligibility. Filing requirements vary by county. Consult an attorney if you have questions.
        </div>
      </footer>
    </div>
  );
    ...
    <div className="text-center max-w-2xl mx-auto">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-left mb-6">
        ...
        <p className="text-blue-800">
          Under N.C.G.S. § 30-15, a surviving spouse is entitled to receive up to <strong>$60,000</strong> from 
          the deceased spouse's personal property for support during the first year after death.
        </p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 text-left mb-6">
        ...
        <ul className="space-y-2 text-slate-700">
          <li className="flex items-start gap-2"><span className="text-emerald-500 mt-1">✓</span> Determine which AOC form version to use</li>
          <li className="flex items-start gap-2"><span className="text-emerald-500 mt-1">✓</span> Identify eligible personal property assets</li>
          <li className="flex items-start gap-2"><span className="text-emerald-500 mt-1">✓</span> Calculate the assignment and any deficiency</li>
          <li className="flex items-start gap-2"><span className="text-emerald-500 mt-1">✓</span> Generate pre-filled AOC-E-100 (and E-101 if needed)</li>
        </ul>
      </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border-2 border-slate-200 hover:bg-slate-50 transition">
              <input type="radio" checked={form.domicileInNC} onChange={() => set('domicileInNC', true)} className="w-5 h-5" />
              <span>Decedent was a resident of this county at death</span>
            </label>
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border-2 border-slate-200 hover:bg-slate-50 transition">
              <input type="radio" checked={!form.domicileInNC} onChange={() => set('domicileInNC', false)} className="w-5 h-5" />
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
          ...
          <div className="bg-white rounded-lg p-4">
            ...
            {isPost ? (
              ...
              <>
                <p className="text-sm text-slate-700">Generally must apply within <strong>one year of death</strong></p>
                {pre2024Passed && (
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="col-span-2 md:col-span-2">
            ...
            <input type="text" value={form.spouseCity} onChange={e => set('spouseCity', e.target.value)}
              className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">State</label>
            <input type="text" value={form.spouseState} onChange={e => set('spouseState', e.target.value)} maxLength={2}
              className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">ZIP</label>
            <input type="text" value={form.spouseZip} onChange={e => set('spouseZip', e.target.value)} maxLength={10}
              className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition" />
          </div>
        </div>
        ...
        <div>
          ...
          <input type="tel" value={form.spousePhone} onChange={e => set('spousePhone', e.target.value)} placeholder="(555) 555-5555"
            className="w-full max-w-xs px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition" />
        </div>
      </div>
      ...
      <div className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-5">
        ...
        <p className="text-amber-800 text-sm">The AOC-E-100 must be signed in front of a notary public before filing.</p>
      </div>
    </div>
  );
}
      ...
      {form.hasPR && (
        <div className="space-y-6 p-6 bg-slate-50 rounded-xl border border-slate-200">
          <div>
            ...
            <input type="text" value={form.prName} onChange={e => set('prName', e.target.value)} placeholder="Personal Representative's name"
              className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition bg-white" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">PR's Address</label>
            <input type="text" value={form.prAddress} onChange={e => set('prAddress', e.target.value)} placeholder="For service of petition"
              className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-slate-400 focus:outline-none transition bg-white" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">
            ...
            <input type="date" value={form.lettersIssuedDate} onChange={e => set('lettersIssuedDate', e.target.value)}
              max={new Date().toISOString().split('T')[0]}
          ...
          {isPost && deadline && (
            <div className={`p-4 rounded-xl ${deadlinePassed ? 'bg-red-50 border border-red-200' : 'bg-emerald-50 border border-emerald-200'}`}>
              ...
              <p className={deadlinePassed ? 'text-red-800' : 'text-emerald-800'}>
                Petition must be filed by: <strong>{deadline.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</strong>
              </p>
          ...
          {isPost && (
            <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl">
              ...
              <p className="text-purple-800 text-sm">You must deliver or mail a copy of the verified petition to the Personal Representative.</p>
            </div>
          )}
        </div>
      )}
...
function AssetsStep({ form, addAsset, updateAsset, removeAsset }) {
  return (
    <div>
      ...
      <p className="text-slate-600 mb-6">Enter the decedent's personal property. Real estate is <strong>not</strong> eligible.</p>
          ...
          return (
            <div key={asset.id} className={`rounded-xl border-2 overflow-hidden ${ineligible ? 'border-red-200 bg-red-50' : 'border-slate-200'}`}>
              <div className="flex justify-between items-center px-4 py-3 bg-slate-50 border-b border-slate-200">
                ...
                <button onClick={() => removeAsset(asset.id)} className="text-red-500 hover:text-red-700 text-xl">×</button>
              </div>
              <div className="p-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" value={asset.value} onChange={e => updateAsset(asset.id, 'value', e.target.value)}
                ...
                <div>
                  ...
                  <input type="text" value={asset.location} onChange={e => updateAsset(asset.id, 'location', e.target.value)}
      ...
      <button onClick={addAsset} className="w-full py-4 rounded-xl border-2 border-dashed border-slate-400 text-slate-600 font-semibold hover:bg-slate-50 transition">
        + Add Asset
      </button>
      ...
      {form.assets.length > 0 && (
        <div className="mt-6 p-4 bg-slate-50 rounded-xl">
          <p className="text-slate-700">
            <strong>Total:</strong> {form.assets.length} asset(s) · {formatCurrency(form.assets.reduce((s, a) => s + (parseFloat(a.value) || 0), 0))}
          </p>
        </div>
      )}
    </div>
    ...
    <div>
              ...
              return (
                <div key={asset.id} className="p-4 flex flex-col md:flex-row md:items-center gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 mt-1">{asset.location || '—'} · Value: {formatCurrency(val)}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-sm text-slate-600">Assign:</span>
                    <div className="flex items-center">
                      <span className="bg-slate-100 px-3 py-2 rounded-l-lg border border-r-0 border-slate-300 text-slate-600">$</span>
                      <input
                        type="number"
                        value={assigned || ''}
                    ...
                    <button onClick={() => assign(asset.id, val)} className="px-3 py-2 text-sm border border-slate-300 rounded-lg hover:bg-slate-50">
                      Full
                    </button>
                  </div>
                </div>
      ...
      {deficiency > 0 && totalAssigned > 0 && totalAssigned < totalEligible && (
        <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
          ...
          <p className="text-red-800 text-sm">Not enough personal property to fully satisfy the allowance. A deficiency judgment (AOC-E-101) will be included.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-slate-500">Decedent:</span><span className="font-medium">{form.decedentName}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">County:</span><span className="font-medium">{form.county}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">File No:</span><span className="font-medium">{form.fileNumber || '(TBD)'}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Date of Death:</span><span className="font-medium">{new Date(form.dateOfDeath).toLocaleDateString()}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Form:</span><span className="font-medium">AOC-E-100 (Rev. {formVer})</span></div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-slate-500">Name:</span><span className="font-medium">{form.spouseName}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Address:</span>
              <span className="font-medium text-right">{form.spouseAddress && `${form.spouseAddress}, `}{form.spouseCity} {form.spouseState} {form.spouseZip}</span>
            </div>
            {form.spousePhone && <div className="flex justify-between"><span className="text-slate-500">Phone:</span><span className="font-medium">{form.spousePhone}</span></div>}
          </div>
        </div>
        ...
        {form.hasPR && (
          <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">Name:</span><span className="font-medium">{form.prName}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Letters Issued:</span><span className="font-medium">{new Date(form.lettersIssuedDate).toLocaleDateString()}</span></div>
              {deadline && <div className="flex justify-between"><span className="text-slate-500">Deadline:</span><span className="font-medium">{deadline.toLocaleDateString()}</span></div>}
            </div>
          </div>
        )}
        ...
        <div className="bg-emerald-50 rounded-xl p-5 border border-emerald-200">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-emerald-700">Statutory:</span><span className="font-bold">{formatCurrency(SPOUSE_ALLOWANCE)}</span></div>
            <div className="flex justify-between"><span className="text-emerald-700">Assigned:</span><span className="font-bold">{formatCurrency(totalAssigned)}</span></div>
            {hasDeficiency && <div className="flex justify-between"><span className="text-red-700">Deficiency:</span><span className="font-bold text-red-700">{formatCurrency(deficiency)}</span></div>}
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100">
              {assigned.map(a => {
                ...
                return (
                  <tr key={a.id}>
                    <td className="px-4 py-3 text-sm">{a.description || ASSET_CATEGORIES.find(c => c.value === a.category)?.label}{isPartial && ' (partial)'}</td>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-6">
        ...
        {ready && (
          <div className="mt-4 p-4 bg-emerald-100 border border-emerald-200 rounded-xl text-emerald-800">
            ✅ <strong>Forms generated!</strong> In production, your download would start automatically.
            <br /><span className="text-sm">(This demo shows the complete UI flow. Actual PDF generation uses pdf-lib with bundled AOC templates.)</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
        <strong>⚠️ Final Reminder:</strong> This tool generates pre-filled forms. You must verify all information before filing. 
        This is not legal advice. Filing requirements vary by county. Consult an attorney if you have questions.
      </div>
    </div>
  );
}
